...
static void syncbb_scan(bool ir_scan, enum scan_type type, uint8_t *buffer, int scan_size)
{
	...
	unsigned mask_last_bit = 1 << ((scan_size-1)%8);
	bool last_tdi = !!(buffer[pos_last_byte] & mask_last_bit);
	...
	bool last_tdo = dirtyjtag_clk_gettdo(1, last_tdi);
	if (type != SCAN_OUT) {
		if (! last_tdo)
			buffer[pos_last_byte] &= ~mask_last_bit;
		else
	 		buffer[pos_last_byte] |= mask_last_bit;
	}
